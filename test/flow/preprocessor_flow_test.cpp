...
DefineGlobalTestSuiteFor(PreporcessorFlow)
    ...
    DefineGlobalTest(ShouldRemoveIfs) {
        auto file_path = TestData::get_data_file_path("if_line.asm");
        auto out_path = TestData::get_data_file_path("if_line.out.asm");
        auto flow = AssemblerFactory::create_preprocessor_flow(file_path, out_path);

        flow.run();

        auto out_file = TestData::get_data_file_content("if_line.out.asm");
        auto expected = TestData::get_data_file_content("if_line.expected.asm");
        ASSERT_EQ(out_file, expected);
    }
